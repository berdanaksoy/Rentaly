...
namespace Rentaly.BusinessLayer.ValidationRules.CarValidations
{
    public class UpdateCarValidator : AbstractValidator<UpdateCarDto>
    {
        ...
        public UpdateCarValidator(ICarDal carDal)
        {
            _carDal = carDal;

            RuleFor(x => x.CarId)
                .GreaterThan(0).WithMessage(ErrorMessages.CarIdRequired);
        ...
        private async Task<bool> BeUniquePlateAsync(UpdateCarDto dto, string plateNumber, CancellationToken token)
        {
            return !await _carDal.AnyAsync(x => x.PlateNumber == plateNumber && x.CarId != dto.CarId);
        }

        private async Task<bool> BeUniqueVinAsync(UpdateCarDto dto, string vin, CancellationToken token)
        {
            return !await _carDal.AnyAsync(x => x.VIN == vin && x.CarId != dto.CarId);
        }
    }
}